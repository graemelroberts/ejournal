...
$urlparams = array( 'id' => required_param('id', PARAM_INT),
		    ...
		    'action' => optional_param('action', '', PARAM_TEXT),
                    'ejournal_id' => optional_param('ejournal_id', '', PARAM_INT),
		    'ereflect_id' => optional_param('ereflect_id', '', PARAM_INT),
    		    'student_id' => optional_param('student_id', '', PARAM_INT),
                    'order_by' => optional_param('order_by', '', PARAM_TEXT) );
...
if ($urlparams['id'])
{
    ...
    $cm     = get_coursemodule_from_id('ejournal', $urlparams['id'], 0, false, MUST_EXIST); // gets coursemodule description based on the id of the coursemodule    
    $course = $DB->get_record('course', array('id' => $cm->course), '*', MUST_EXIST);
    $gr  	= $DB->get_record('ejournal', array('id' => $cm->instance), '*', MUST_EXIST);	
} elseif ($n) {
    $gr		= $DB->get_record('ejournal', array('id' => $urlparams['n']), '*', MUST_EXIST);
...
$url = new moodle_url('/mod/ejournal/view.php', $urlparams);
...
require_capability('mod/ejournal:view', $context);
...
$ejournal = new ejournal($context, $cm, $course);
